...
def _extract_device_schedule(device: dict) -> dict[str, Any] | None:
    ...
    return min(
        valid_schedules,
        key=lambda schedule: _WEEKDAY_ORDER.get(schedule.get("dayOfWeek", ""), 99),
    )


def _is_charging_device(device: dict) -> bool:
    """Return whether a device is relevant for EV charging sensors."""
    return device.get("deviceType") in _CHARGING_DEVICE_TYPES
            ...
            if devices:
                ...
                for device in devices:
                    ...
                    if device_id:
                        ...
                        if _is_charging_device(device):
                            entities.append(
                                OctopusNextDispatchStartSensor(
                                    acc_num, coordinator, device_id
                                )
                            )
                            entities.append(
                                OctopusNextDispatchEnergySensor(
                                    acc_num, coordinator, device_id
                                )
                            )
                            entities.append(
...
class OctopusChargingDeviceSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, account_number, coordinator, device_id: str) -> None:
        ...
        self._norm_name = _normalize_name(self._device_name)
        self._attr_has_entity_name = False
    ...
    def _get_last_session(self) -> dict | None:
        ...
        sessions_with_start = [
            ...
            if _parse_api_datetime(session.get("start")) is not None
        ]
        ...
        return max(
            sessions_with_start,
            key=lambda session: _parse_api_datetime(session.get("start"))
            or datetime.min.replace(tzinfo=timezone.utc),
        )
    ...
    def _get_next_dispatch(self) -> dict | None:
        ...
        for dispatch in dispatches:
            ...
            start = _parse_api_datetime(dispatch.get("start"))
...
class OctopusNextDispatchStartSensor(OctopusChargingDeviceSensor):
    ...
    def native_value(self) -> datetime | None:
        ...
        return _parse_api_datetime(dispatch.get("start")) if dispatch else None
